...
- [x] 浏览模组（筛选栏按分类 / 游戏版本 / 排序组合筛选，滚动到底自动加载下一页）
- [x] 搜索模组（允许聚合搜索）
...
> **请勿使用裸的 `flutter build apk`**：
> Android 的默认行为是打「胖 APK」，把 arm64-v8a/armeabi-v7a/x86_64 三种架构的原生库全塞进同一个包，
> 请按上面的命令加上`--target-platform` 或 `--split-per-abi`。
